...
#pragma warning disable CS0626 // Method, operator, or accessor is marked external and has no attributes on it
        extern public Dictionary<String, String> Results { get; set; }
#pragma warning restore CS0626 // Method, operator, or accessor is marked external and has no attributes on it
	...
	public Dictionary<String,String> DoScan(String ipString)
	{
		...
		{
			MacState state = new MacState() {Results = Results};
			if (ArpScanner.IPv4Tools.IsIPRangeFormat(ipString))
		    {
				ArpScanner.IPv4Tools.IPRange iprange = IPv4Tools.IPEnumerator[ipString];
				
		        foreach (var n in iprange)
					state.Counter++;

				foreach (var ip in iprange)
					ThreadPool.QueueUserWorkItem(GetMAC, new IPQueryState(state) {Query = ip} );
				
				state.DoneEvent.WaitOne();
		    }
		    else
		    	GetMAC(new IPQueryState(state) {Query = ipString});
	    ...
	    class IPRangeEnumerator : IEnumerator<string>
	    {
	        ...
	        public void ProcessIPRange(Match rangeMch)
	        {
	            ...
	            { 
	                ...
	                var ipuint = System.BitConverter.ToUInt32(arrIpBytes, 0);
	                _loAddr = ipuint;
